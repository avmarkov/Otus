...
> Следующий запрос реализует кросс соединение. Сначала формируется таблица на основе внутреннего соединения (оператор SQL INNER JOIN).
...
> 
> В данном запросе реализовано полное соединение. Т.е сначала отобразились строки, для которых выполняется условие соединения. Потом добавились модели, для которых не указан производитель, затем производители, для которых нет соответсвующих моделей и, наконец, типы кузовов, которых не ни у одной модели.
