...
> 
> И вообще говоря, в схеме витрина+триггер при UPDATE или DELETE тоже будет такая же проблема, которая решается также добавлением цены товара good_price в непосредственно в таблицу sales. Причем нужно хранить еще и текущую цену в таблице goods, чтобы правильно вычилслить сумму при UPDATE или DELETE. А вот в случае INSERT в схеме витрина+триггер проблемы нет, работать будет корректно.