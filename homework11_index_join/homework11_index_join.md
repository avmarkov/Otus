...
> В данном запросе реализовано полное и прямое соединение. Не смотря на то, что первое соединеие - full, сначала вывелись у нас все записи удовлетворяющие всем соединеням и только те модели, для которых есть соответсвующий тип кузова. Причем не обязательно, чтобы у модели был производитель. Что мы и видим в результате этого запроса. 